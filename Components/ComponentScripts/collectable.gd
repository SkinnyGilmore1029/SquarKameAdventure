...
## Nested Dictionary Keys [Name, Use Signal, Has Args, Args, UiSignal][br]
## Last three might be false, null, null
var node_hub: Dictionary = {
	Keys : {
		"Name" : "Key",
		"Use Signal" : SignalHub.key_collected,
		"Has Args" : false,
		"Args" : null,
		"UiSignal" : null,
	},
	OneUps: {
		"Name" : "OneUps",
		"Use Signal" : SignalHub.one_up_collected,
		"Has Args" : true,
		"Args" : [PlayerGlobals, "players_lives"],
		"UiSignal" : SignalHub.one_up_global
	}
}
...
func check_parent() -> void:
	...
	var collectable_signal = collectable_data.get("Use Signal")
	...
	var the_args = collectable_data.get("Args")
	...
	if collectable_data.get("UiSignal") != null:
		collectable_data.get("UiSignal").emit()
